...
import {
  LayoutDashboard, Upload, AlertTriangle, ClipboardCheck,
  FileSpreadsheet, Calculator, Download, ShieldCheck,
} from "lucide-react";
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex">
      ...
      <aside className="w-64 bg-white border-r border-slate-200 fixed inset-y-0 left-0 flex flex-col">
        ...
        <div className="p-4 border-t border-slate-200 text-[11px] text-slate-400">
          Encryption: <span className="text-emerald-600 font-medium">AES-256-GCM</span><br />
      ...
      <div className="ml-64 flex-1 flex flex-col">
        <header className="h-16 sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-slate-200 flex items-center px-8 gap-4">
          <div className="flex items-center gap-4 flex-1">
            <div>
              <label className="block text-[11px] uppercase tracking-wider text-slate-500 mb-0.5">Seller GSTIN</label>
              <select
              ...
              >
                {sellers.map((s) => (
                  <option key={s.gstin} value={s.gstin}>{s.gstin} — {s.trade_name || s.legal_name}</option>
                ))}
              </select>
            </div>
            <div>
              <label className="block text-[11px] uppercase tracking-wider text-slate-500 mb-0.5">Filing Period</label>
              <input
