...
export default function Exceptions() {
  ...
  useEffect(() => { load(); }, [sellerGstin, period]); // eslint-disable-line react-hooks/exhaustive-deps

  const resolve = async (id, corrected) => {
  ...
  return (
    <div className="space-y-6" data-testid="exceptions-page">
      <div>
        <h1 className="font-display text-4xl sm:text-5xl font-semibold tracking-tight">Exception Ledger</h1>
        <p className="mt-2 text-slate-500">Rows the parser couldn&apos;t classify. Fix inline — nothing is silently dropped.</p>
      </div>
        ...
        <CardContent className="pt-0">
          <table className="w-full text-sm">
            ...
            <tbody data-testid="exceptions-tbody">
              {rows.length === 0 && (
                <tr><td colSpan={6} className="py-10 text-center text-slate-400">No open exceptions. All rows landed cleanly.</td></tr>
              )}
              {rows.map((r) => {
                const fix = editing[r.id] ?? { supplier_gstin: r.raw?.["Buyer GSTIN"] || r.raw?.supplier_gstin || "" };
                return (
                  <tr key={r.id} className="border-b border-slate-100 align-top">
                    <td className="py-2 px-3 font-mono text-xs text-slate-500">{r.upload_id?.slice(0, 8) || "—"}</td>
                    ...
                    <td className="py-2 px-3">
                      <details>
                        ...
                        <pre className="mt-2 text-[10px] text-slate-500 bg-slate-50 border border-slate-200 rounded p-2 max-w-md whitespace-pre-wrap overflow-x-auto">{JSON.stringify(r.raw, null, 2)}</pre>
                      </details>
                    </td>
                    <td className="py-2 px-3">
                      <Input
                        ...
                        onChange={(e) => setEditing((prev) => ({ ...prev, [r.id]: { supplier_gstin: e.target.value } }))}
