...
export default function UploadPage() {
  ...
  // Poll job status every 2s while any is not terminal
  useEffect(() => {
  ...
  const uploadFiles = useCallback(async (files) => {
    ...
    for (const file of files) {
      ...
      const isXlsx = file.name.toLowerCase().endsWith(".xls") || file.name.toLowerCase().endsWith(".xlsx");
  ...
  return (
    <div className="space-y-8" data-testid="upload-page">
      <div>
        <h1 className="font-display text-4xl sm:text-5xl font-semibold tracking-tight">Dropzone Control Center</h1>
        <p className="mt-2 text-slate-500">Drag CSV/XLSX marketplace settlements or vendor invoice PDFs. Async pipeline handles OCR, parsing and routing.</p>
      </div>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardContent className="p-6">
          <div className="flex flex-wrap items-center gap-4 mb-4">
            <div>
              <label className="block text-[11px] uppercase tracking-wider text-slate-500 mb-1">Marketplace (for CSV/XLSX)</label>
                ...
                {MARKETPLACES.map((m) => (
                  <button
                    ...
                    className={cls(
                      ...
                      marketplace === m.id ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-800"
                    )}
                  >{m.label}</button>
                ))}
              </div>
            ...
            <div className="ml-auto text-xs text-slate-500">
              Files encrypted at rest with AES-256-GCM · Download URLs are signed and expire in 10 min.
            </div>
          </div>
          ...
          <div
            ...
            className={cls(
              ...
              dragActive ? "border-blue-500 bg-blue-50/40 dropzone-active" : "border-slate-300 bg-slate-50 hover:border-blue-400 hover:bg-blue-50/30"
            )}
          >
            <UploadCloud className={cls("w-10 h-10 mx-auto mb-3", dragActive ? "text-blue-600" : "text-slate-400")} />
            <div className="font-display text-lg text-slate-800 font-medium">Drop files here, or click to browse</div>
            <div className="text-sm text-slate-500 mt-1">CSV, XLSX (marketplace) · PDF (vendor invoices)</div>
            <input
              ref={inputRef}
              type="file"
              multiple
        ...
        <CardContent className="pt-0">
          <table className="w-full text-sm">
            ...
            <tbody data-testid="jobs-tbody">
              {jobs.length === 0 && (
                <tr><td colSpan={6} className="py-8 text-center text-slate-400">No jobs yet.</td></tr>
              )}
              {jobs.map((j) => (
                <tr key={j.id} className="border-b border-slate-100 table-row-hover">
                  <td className="py-2 px-3 font-mono text-xs text-slate-600">{j.id.slice(0, 8)}</td>
                  <td className="py-2 px-3">{j.kind === "vendor_ocr" ? "Vendor PDF" : "Marketplace file"}</td>
                  <td className="py-2 px-3"><StatusBadge status={j.status} /></td>
                  <td className="py-2 px-3 w-56">
                    <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                      <div className={cls("h-full transition-all",
                        j.status === "failed" ? "bg-red-500" : j.status === "exception" ? "bg-amber-500" : j.status === "complete" ? "bg-emerald-500" : "bg-blue-500")}
                        style={{ width: `${j.progress}%` }} />
                    </div>
                  </td>
                  <td className="py-2 px-3 text-slate-600 text-xs">{j.message}</td>
                  <td className="py-2 px-3 text-right tabular">{j.attempts}/{j.max_attempts}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
const SampleTemplates = ({ onSelect, marketplace }) => (
  <div className="mt-4 flex items-center gap-3 text-xs text-slate-500">
    ...
    <span className="inline-flex items-center gap-1 text-slate-400">
      <Package className="w-3.5 h-3.5" /> pipelines run async via BackgroundTasks + MongoDB job status
    </span>
  </div>
);
