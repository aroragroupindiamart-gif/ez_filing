...
export default function ExportPage() {
  ...
  const [previews, setPreviews] = useState<Record<string, any>>({ gstr1: null, gstr3b: null });

  const buildAndDownload = async (kind: string) => {
    if (!sellerGstin) {
    ...
    <div className="space-y-6" data-testid="export-page">
      <div>
        ...
        <p className="mt-2 text-slate-500 max-w-3xl">
          Payload stored encrypted at rest (AES-256-GCM). Download URLs are HMAC-signed and expire
          in 10 minutes.
        </p>
      </div>
