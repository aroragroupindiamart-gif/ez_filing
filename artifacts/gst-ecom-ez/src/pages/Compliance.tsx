...
export default function Compliance() {
  ...
  const [imsList, setImsList] = useState<any[]>([]);
  ...
  const openDrill = (cellKey: string, ids: string[]) => {
  ...
  const exportBoth = async () => {
    try {
      ...
      setTimeout(
        () => window.open(api.downloadUrl(g3b.export_id, g3b.download_token), "_blank"),
        400
      );
    ...
    <div className="space-y-6" data-testid="compliance-page">
      <div className="flex items-baseline justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-500">
            Side-by-side GSTR-1 and GSTR-3B (3.1 + Table 4) — click any Table 4 cell to drill down
            to source docs.
          </p>
        </div>
        <Button
      ...
      <Tabs defaultValue="split">
        <TabsList data-testid="compliance-tabs">
          ...
          <TabsTrigger value="gstr1" data-testid="tab-gstr1">
            GSTR-1
          </TabsTrigger>
          <TabsTrigger value="gstr3b" data-testid="tab-gstr3b">
            GSTR-3B
          </TabsTrigger>
        </TabsList>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-4">
            <div className="space-y-4">
              ...
              <Section title="B2B" testid="section-b2b">
                ...
                <Row k="Total taxable" v={fmtINR(g1.b2b.reduce((s: number, r: any) => s + r.val, 0))} />
              </Section>
              <Section title="B2CS (Summary)" testid="section-b2cs">
                <Row k="Groups" v={g1.b2cs.length} />
                <Row
                  k="Total taxable"
                  v={fmtINR(g1.b2cs.reduce((s: number, r: any) => s + r.txval, 0))}
                />
              </Section>
                  ...
                  return (
                    <button
                      ...
                      className={cls(
                        ...
                        hasSrc
                          ? "hover:border-blue-300 hover:bg-blue-50/40 border-slate-100 cursor-pointer"
                    ...
                    >
                      ...
                      <div className="mt-1 text-[11px] text-slate-400 tabular">
                        IGST {fmtINR(cell.igst)} · CGST {fmtINR(cell.cgst)} · SGST{" "}
                        {fmtINR(cell.sgst)}
                        {hasSrc ? ` · ${cell.source_ids.length} source(s)` : ""}
                      </div>
                    </button>
                  );
                })}
...
const RawJson = ({ title, data }: { title: string; data: any }) => (
...
const Drilldown = ({
  ...
  <div
    className="fixed inset-0 z-40 flex items-end sm:items-center justify-center bg-slate-900/40 p-4"
    onClick={onClose}
    data-testid="drilldown-modal"
  >
    <div
      className="bg-white rounded-xl shadow-xl border border-slate-200 max-w-4xl w-full max-h-[80vh] overflow-hidden flex flex-col"
      onClick={(e) => e.stopPropagation()}
    >
      <div className="px-5 py-3 border-b border-slate-200 flex items-center justify-between">
        <div>
          <div className="text-[11px] uppercase tracking-wider text-slate-500">
            Drill-down · Table {cellKey}
          </div>
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-slate-100">
                <td className="py-2 px-3 font-mono text-xs">
                  {r.invoice_number || r.credit_note_number}
                </td>
                ...
                <td className="py-2 px-3 text-right tabular">
                  {fmtINR(r.tax_amount ?? (r.igst + r.cgst + r.sgst))}
                </td>
                <td className="py-2 px-3 text-xs text-slate-500">
                  {r.itc_ineligible_reason ||
                    r.decision ||
                    (r.verified_gstin ? "GSTIN verified" : "GSTIN unverified")}
                </td>
              </tr>
            ))}
          </tbody>
