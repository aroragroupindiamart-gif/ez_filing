  ...
  return (
    <Card className="border-slate-200 shadow-sm" data-testid={testid}>
      <CardContent className="p-5">
        ...
        <div className={`mt-2 text-3xl font-display font-semibold tabular ${toneMap[tone]}`}>
          {value}
        </div>
      ...
      <div className="flex items-baseline justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-500 text-base">
            {periodLabel(period)} · seller {sellerGstin} — reconcile, review, export portal-ready
            JSON.
          </p>
        </div>
                ...
                {jobs.length === 0 && (
                  <tr>
                    <td colSpan={4} className="py-6 text-center text-slate-400">
                      No jobs yet. Upload marketplace or vendor files to begin.
                    </td>
                  </tr>
                )}
                {jobs.map((j) => (
        ...
        <Card className="border-slate-200 shadow-sm">
          ...
          <CardContent className="space-y-2 pt-0">
            <ShortLink
              to="/compliance"
              icon={FileText}
              label="GSTR-1 & 3B preview"
              tone="primary"
              testid="short-compliance"
            />
            <ShortLink
              to="/ims"
              icon={ClipboardCheck}
              label="IMS action tracker"
              testid="short-ims"
            />
            <ShortLink
              to="/exceptions"
              icon={AlertTriangle}
              label={`Exception ledger (${exceptions.length})`}
              tone={exceptions.length ? "warn" : "default"}
              testid="short-exceptions"
            />
            <ShortLink
              to="/interest"
              icon={ShieldCheck}
              label="Interest estimator"
              testid="short-interest"
            />
          </CardContent>
        </Card>
      </div>
...
export const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs border font-medium ${map[status] || map.queued}`}
        ...
        {["4A", "4B(1)", "4B(2)", "4D"].map((k) => {
          ...
          return (
            <div key={k} className="p-5" data-testid={`itc-cell-${k}`}>
              ...
              <div className="mt-3 text-2xl font-display font-semibold tabular">
                {fmtINR(total)}
              </div>
