...
from models import VendorInvoice, is_valid_gstin

STORAGE_DIR = Path(os.environ.get("STORAGE_DIR", "/app/backend/storage"))
...
async def run_vendor_ocr(job_id: str) -> None:
    job = await repo.get_job(job_id)
    if not job:
        return
    upload = await repo.get_upload(job["upload_id"])
